...
- Incompleteness is local: $G_F$ acts on $Q_t$ only. $\Gamma(\mathrm{global\_shadow})$ is a projection, so a global Gödel sentence never forms. $G_F$ is a computable, local transformation on the language of a slice that removes or refuses any formula whose quantificational reach or self‑reference would collapse the knot into a static universe.
