...
519.8 Dynamical systems / differential equations / mathematical modeling

RDG can be treated as a state‑determined formalism: 

	the state (S) is the system, 

and all future behavior is generated from it through the MFE flux engine.This document defines a "State‑Determined System" in the RDG/MFE formalism: the full instantaneous state S contains everything required to generate future behavior via an evolution operator (RME/MFE). In compact form: S(t+Δt) = Φ(S(t)).
