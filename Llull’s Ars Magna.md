...
A *typed atom* is a pair $(\lambda,s)$. The language is generated by composing atoms of possibly different sorts; the most frequent well-formed propositions are those in which the first atom is read as subject (usually a Dignity or Subject) and the second as predicate (Dignity, Relative, Virtue, \ldots).
...
The great Table then inserts the marker $T$ in every possible slot and mixes columns of Figure A with columns of Figure T, producing twenty typed readings of each of the 84 triples (1680 compartments, many of them semantically redundant).
...
- Question + Dignity + Relative: the typed triple $BCTB$ reads “Utrum Bonitas sit Differentia?”.
...
where $\rho_*$ is the corresponding permutation of the generated propositions. When letters are drawn from *different* columns the rotation still generates new questions, but the semantic content is no longer a mere relabeling; it is a new typed combination.
...
Treat each typed atom as an object of an  and each composition as an edge. The Structure triad (Structure–Interaction–Dynamics) and the Power triad (Power–Evaluation–Dynamics) then become two parallel readings of the same letter:
...
The resulting picture is a finite, typed, rotationally generated magma of propositions whose enumerative skeleton is classical combinatorics, whose symmetry is a cyclic group action, and whose semantics is a many-sorted interpretation that can be transported, letter by letter, into an Structure–Power operator system.
