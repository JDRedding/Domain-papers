...
$$
H \propto d(E,U) \cdot R
$$

- $H$: felt humor  
- $d(E,U)$: distance / clash between expected and unexpected interpretations  
- $R$: resolution (the mind can *reframe* the clash, not just be confused)

If there is clash but no resolution, you get nonsense or discomfort, not a joke.

**Benign violation (McGraw & Warren style)**  
Humor when something is wrong *and* safe:

$$
H = \begin{cases} \text{high} & \text{if } V \land B \\ \text{low} & \text{otherwise} \end{cases}
$$

- $V$: perceived violation (norm, logic, dignity, piety)  
- $B$: benign context (play frame, distance, fiction, self-target)

Satire often raises $V$ while keeping just enough $B$ that the audience stays in the room.

**Superiority (Hobbesian sketch)**  
Laughing *down*:

$$
H \propto (S_{\text{self}} - S_{\text{target}})
$$

- $S$: status / competence as perceived in the moment  

American trickster humor often flips this: the target looks high-status, the speaker looks low-status, then the inequality reverses.

**Relief / pressure**  
Crude hydraulic picture (Freud-ish, not quantitative):

$$
H \propto \Delta P = P_{\text{built-up}} - P_{\text{after release}}
$$
...
$$
X_{n+1} = k\, X_n, \quad k > 1
$$

Paul Bunyan height, Babe’s thirst, the size of the camp breakfast: each telling multiplies the last. American exaggeration is geometric, not additive.

**Gap between creed and practice** (the usual American satirical engine)
...
- $R$: observed reality  
- Satire scales with $G$, then multiplies by $X$:

$$
\text{American satire} \propto X \cdot G
$$
...
$$
H = \frac{\text{atrocity or folly}}{\text{calmness of voice}}
$$

The flatter the delivery, the sharper the cut.

**SNL / Daily Show timing**

$$
H \propto \frac{\text{recognition of the week’s event}}{\text{seconds until the twist}}
$$
