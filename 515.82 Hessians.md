# Hessians
- 515.82 — Numerical differentiation
- 515.825 — Hessians & curvature
- 515.828 — Optimization algorithms (Newton, quasi‑Newton)
...
Hessians are the mathematical core of Newton’s method, quasi‑Newton methods, and second‑order optimization in general. They measure local curvature, and Newton‑type algorithms are exactly the class of methods that use curvature to accelerate optimization. But Hessians also appear in Gauss–Newton, Fisher information, K‑FAC, natural gradient, spectral analysis, and stochastic curvature estimation, all of which are Newton‑adjacent but not strictly Newton’s method.
...
#### Fisher Information and Its Connection to the Hessian

The **Fisher information matrix** is another curvature operator closely related to the Hessian of the negative log‑likelihood. It appears in statistical estimation, natural‑gradient methods, K‑FAC, and influence‑function analysis. In many machine‑learning settings, the Fisher serves as a *positive‑semidefinite surrogate* for the Hessian, especially when the loss is a log‑likelihood.

##### Empirical Approximation: Fisher ≈ Hessian

When the model distribution $\(p_\theta(y\mid x)\)$ is close to the true data distribution $\(p^\*(y\mid x)\)$, the expectation over $\(p_\theta\)$ can be approximated by the empirical labels $\(y_i\)$. This yields:

```math
F(\theta)
\approx
-\frac{1}{n}\sum_{i=1}^n
\nabla_\theta^2 \log p_\theta(y_i\mid x_i)
= H(\theta),
```

where \(H(\theta)\) is the Hessian of the negative log‑likelihood.
...
When the loss is $\(-\log p_\theta(y\mid x)\)$, **Fisher and Hessian coincide up to sampling noise**.

##### Fisher vs. Hessian: Practical Differences

Even when they are close, the two matrices have distinct properties:

- **Fisher is always positive semidefinite**, since it is an expectation of outer products.  
- **Hessian may be indefinite**, especially in deep networks.  
- Fisher depends on the *model distribution*; Hessian depends on the *observed labels*.  
- Fisher is central to **natural gradient** and **information geometry**; Hessian is central to **Newton’s method**.

In practice, both encode curvature, but the Fisher is often smoother and more stable.

The Fisher information matrix is a curvature operator that equals the **expected** Hessian of the negative log‑likelihood. When the model distribution matches the data distribution, the empirical Hessian approximates the Fisher. This makes the Fisher a central object in second‑order optimization, natural gradient methods, and influence‑function analysis.
...
These methods provide practical ways to approximate or exploit curvature in optimization, reducing the naive $\(O(d^2)\)–\(O(d^3)\)$ cost of full Newton steps to linear or near‑linear cost in the number of parameters. In optimization, Hessians are fundamentally Newtonian.   They are the mathematical engine behind Newton’s method and all its modern descendants (Gauss–Newton, natural gradient, K‑FAC, L‑BFGS, AdaHessian, etc.). But Hessians also have a life outside Newton’s method, they are general curvature operators in analysis, geometry, and physics.

## Notation
The core Hessian notation set

- $\(H = \nabla^2 L\)$ — Hessian  
- $\(H_{ij} = \partial^2 L / (\partial \theta_i \partial \theta_j)\)$ — components  
- $\(Hv\)$ — Hessian–vector product  
- $\(\{diag}(H)\)$ — diagonal  
- $\(\{Tr}(H)\)$ — trace  
- $\(J^\top H_z J\)$ — Gauss–Newton  
- $\(F\)$ — Fisher  
- $\(s_k, y_k\)$ — quasi‑Newton curvature pairs  
...
stochastic curvature estimators
...
K‑FAC, 
diagonal curvature estimators (AdaHessian)

515.828 — Optimization algorithms
