...
- 324.973 Elections & voting (United States)
  320.9 Political situation and conditions 
- 320.55 Political ideologies
- 511.8 Graph theory & network analysis

This note develops a compact, mathematically grounded framework for measuring and visualizing how individual voters relate to political parties over time. It focuses on providing clear notation, a small set of relational primitives, and extensible components (e.g., layered policy domains and voter-specific metrics) to support empirical measurement, simulation, and comparative analysis rather than prescribing political outcomes.
...
- Regions where the field is nearly equal for both parties — hybrid band.
...
# **FULL INDEX (Alphabetical, Cross‑Referenced)**

```
A
---------------------------------------------------------------
Alignment space (𝒜)
    The combined geometric space of identity, policy, and tone:
    𝒜 = 𝓘 × 𝓟𝓸 × 𝓣. Voters and parties are points in this space.

Alignment score A(V,P)
    Weighted sum of identity, policy, and tone alignment.

Alignment difference Δ
    Δ(V) = |A(V,D) − A(V,R)|, measuring preference strength.

Argmax decision rule
    The voter selects the party with maximal alignment:
    P* = argmax_P A(V,P).
...
---------------------------------------------------------------
Bisector region
    Region of 𝒜 where Δ is small; voter is equidistant from both parties.

β_k (policy-domain weights)
    Weights for governance, economic, social/cultural, foreign domains.


C
---------------------------------------------------------------
Composite score
    The overall alignment score A(V,P).

Cosine similarity
    Directional similarity measure for identity/tone vectors.

Critical point (party flip)
    A time t where A(V,D) = A(V,R), causing preferred party to change.


D
---------------------------------------------------------------
Decision rule
    Formal rule selecting the party with highest alignment.

Dynamic trajectories V(t), P(t)
    Time-dependent positions of voters and parties in 𝒜.

d_max (policy distance normalization)
    Maximum possible policy distance used to scale Po(V,P) into [0,1].


F
---------------------------------------------------------------
Four-domain policy model
    Policy decomposed into governance, economic, social/cultural, foreign.

Feature vectors (identity, policy, tone)
    Structured representations of voter/party attributes.


G
---------------------------------------------------------------
g_V (voter metric tensor)
    Voter-specific weighting of distances in alignment space.

Governance domain
    Policy subspace covering institutional and structural preferences.


H
---------------------------------------------------------------
Hybrid voters
    Voters with small Δ; neither party strongly dominates.


I
---------------------------------------------------------------
Identity alignment I(V,P)
    Similarity between identity vectors of voter and party.

Identity vector i_V, i_P
    Cultural, demographic, symbolic features encoded as a vector.

Issue weights α_j
    Weights assigned to individual policy issues.


M
---------------------------------------------------------------
Moral-tone alignment T(V,P)
    Similarity between tone vectors of voter and party.

Metric tensor (voter-specific)
    Same as g_V; defines how the voter perceives relational distances.


P
---------------------------------------------------------------
Policy alignment Po(V,P)
    Agreement between voter and party in policy space.

Policy vector v_V, p_P
    Positions in an n-dimensional issue space.

Policy domains (g,e,s,f)
    Governance, economic, social/cultural, foreign-policy subspaces.

Preferred party P*
    Party maximizing alignment score.


R
---------------------------------------------------------------
Relational geometry
    Alignment defined by relations between points, not coordinates.

Relational maps I, Po, T
    Functions mapping pairs of points to alignment scores in [0,1].


S
---------------------------------------------------------------
Similarity function sim(x,y)
    Cosine similarity used for identity and tone.

Social/cultural domain
    Policy subspace covering cultural norms and social values.


T
---------------------------------------------------------------
Tone alignment
    Similarity between stylistic/moral-tone vectors.

Tone vector t_V, t_P
    Features describing communication style and moral framing.

Trajectories over time
    Evolution of voter and party positions in alignment space.
...
### **Q1 — Why separate identity, policy, and tone?**  
Because political alignment is multi‑dimensional.  
A voter may agree with a party on policy but reject its tone, or vice versa.  
Separating axes prevents collapsing distinct motivations into a single number.

### **Q2 — Why use cosine similarity for identity and tone?**  
Cosine similarity captures **directional similarity**, not magnitude.  
Identity and tone are about *kind*, not *intensity*.

### **Q3 — Why treat policy as a distance metric instead of similarity?**  
Policy disagreements are often **positional** (how far apart), not **categorical**.  
Distance captures this more naturally.

### **Q4 — What does the metric tensor $g_V$ do?**  
It encodes how *the voter* weights different dimensions.  
Two voters with identical coordinates may perceive distances differently.

### **Q5 — What does “hybrid voter” mean in this model?**  
A voter whose alignment difference $\Delta$ is small.  
Geometrically:  
They lie near the relational bisector between the two parties.

### **Q6 — Why treat parties as dynamic trajectories?**  
Because party identity, policy, and tone evolve over time.  
Static models miss real‑world shifts.

### **Q7 — What causes a “critical point” flip?**  
A crossing of relational distances:  

$$
A(V,D) = A(V,R)
$$

This can happen because the voter moves, the party moves, or both.

### **Q8 — Why decompose policy into four domains?**  
Because voters often weight domains differently.  
A voter may be economically aligned with one party but socially aligned with another.

### **Q9 — Why call this “relational geometry”?**  
Because alignment is defined by **relations between points**, not by absolute positions.  The model is fundamentally about **maps**, not coordinates.

### **Q10 — How does this model relate to real elections?**  
It describes **alignment**, not outcomes.  
It helps explain why voters choose parties, but does not predict election results.  
