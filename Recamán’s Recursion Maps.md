...
This note reframes Recamán’s sequence as a three‑layer dynamical system (RDG–MFE–Q): a Relational Dynamic Geometry (state/visited geometry), a Momentum Flux Engine (the propose subtract/add n rule), and a Q admissibility predicate (self‑avoidance + positivity). It gives both a compact recurrence and a cellular‑automaton (1D lattice) encoding, then extends the model by adding per‑site energy and survival fields so Q can enforce energetic/stability constraints; Recamán’s original rule is the degenerate, energy‑agnostic case.
